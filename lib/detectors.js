 ...
 * toast on older editors): "Change key" for a bound action, or "Assign
 * shortcut" for an unbound one — both call `ctx.ui.openKeyboardShortcuts(actionId)`,
 * which jumps straight into rebind capture for that row whether or not it
 * already has a key; both pair with a "Don't show again" that mutes the
 * action for good.
 */
...
export function createEngine(ctx, state) {
  ...
  function nudge(actionId) {
    ...
    if (kb.key) {
      // The close (×) button already dismisses the toast, so the primary slot
      // is spent on "Change key" instead of a redundant "Dismiss" — jumps
      // straight into rebind capture for this row, even though one's already set.
      const primaryAction =
        typeof ctx.ui.openKeyboardShortcuts === "function"
          ? { label: ctx.i18n.t("Change key"), onClick: () => ctx.ui.openKeyboardShortcuts(actionId) }
          : { label: ctx.i18n.t("Dismiss"), onClick: () => {} };
      ctx.ui.showToast({
